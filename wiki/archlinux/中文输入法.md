...
接下来的内容稍微涉及了一点输入法工作原理，如果觉得有些难理解的话可以让 AI 辅助进行阅读，如果不想看的话可以按照下面这个代码块的命令进行设置，出问题再说：


```
# 创建用户家里的环境变量配置目录，编辑输入法环境变量
...
1. 了解如何让输入法正常工作
   ...
   让输入法正常工作主要有两种方式，其一是用输入法模块走 D-Bus 总线跟输入法通信的传统方式，其二是用 Wayland 协议以 Wayland 合成器为中间人跟输入法通信的现代方式。
   ...
   对于后者，软件需要适配 Wayland 的 `text-input` 协议，输入法框架要支持`input-method`协议，合成器同时支持这两类协议，通常无须配置环境变量输入法就能正常工作。`text-input`协议和`input-method`协议的版本必须匹配，但通常无须在意这个事情。举个例子，GNOME 的文本编辑器支持 `text-input-v3`，fcitx5 支持 `input-method-v2`，Niri 同时支持 `text-input-v3` 和 `input-method-v2`，输入法就可用了。
...
3. 正确设置环境变量
    ...
    >`cat`命令打印文件内容；`grep`命令过滤输出内容

    ![](../pictures/输入法环境变量.png)
