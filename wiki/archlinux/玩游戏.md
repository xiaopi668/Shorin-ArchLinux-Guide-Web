...
- [使用EasyTier组建虚拟局域网](玩游戏.md#使用easytier组建虚拟局域网)
---
...
> 完整教程见 [虚拟局域网](./虚拟局域网.md)

想和远方的朋友联机玩游戏，但双方都没有公网 IP？[EasyTier](https://github.com/EasyTier/EasyTier) 可以把不同地点的设备组成一个虚拟局域网，让联机游戏像在同一个局域网内一样互相连接。安装：

    sudo pacman -S easytier

组网很简单，所有设备运行相同网络名称（`--network-name`）和网络密码（`--network-secret`）的 `easytier-core`，再通过一个公共服务器就能互通：
...
另一台设备把 `-i` 换成其他虚拟 IP 后运行相同命令即可加入，互相 ping 通虚拟 IP 后就能联机了。
